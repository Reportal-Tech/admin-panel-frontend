...
export async function newBuilder(formData: FormData) {
  ...
  try {
    const registerResponse = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/v1/admin/createBuilder`,
      {
        method: 'POST',
        headers: {
...
export async function fetchBuilderStatus() {
  ...
  try {
    const buildersResponse = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/v1/admin/getBuilderStatus`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${authToken}`,
...
export async function updatePendingBuilder(
  ...
  try {
    const updateVerificationStatus = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/v1/admin/updateBuilderStatus/${builderId}`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
